...
function setupDragAndDrop(fruit) {
    fruit.addEventListener('pointerdown', (e) => {
        ...
        // 锁定 gameArea 滚动，防止系统把拖拽手势判断为页面滚动
        gameArea.style.overflowY = 'hidden';
        gameArea.style.touchAction = 'none';

        const fruitRect = fruit.getBoundingClientRect();
        const gameRect = gameArea.getBoundingClientRect();

        // 将水果挂到 gameArea，坐标加上 gameArea 的滚动偏移
    ...
    fruit.addEventListener('pointerup', (e) => endDrag(fruit, e));

    // 拖拽被系统打断时（来电、切换应用等）也能正确还原
    fruit.addEventListener('pointercancel', (e) => endDrag(fruit, e));
}
