...
function setupDragAndDrop(fruit) {
    fruit.addEventListener('pointerdown', (e) => {
        ...
        // 将水果重新挂载到 gameArea，使其可以自由浮动在袋子和水果区之上
        const fruitRect = fruit.getBoundingClientRect();
        const gameRect = gameArea.getBoundingClientRect();

        gameArea.appendChild(fruit);
        fruit.style.position = 'absolute';
        fruit.style.left = (fruitRect.left - gameRect.left) + 'px';
        fruit.style.top = (fruitRect.top - gameRect.top) + 'px';
    ...
    fruit.addEventListener('pointermove', (e) => {
        ...
        const gameRect = gameArea.getBoundingClientRect();
        fruit.style.left = (e.clientX - dragOffsetX - gameRect.left) + 'px';
        fruit.style.top = (e.clientY - dragOffsetY - gameRect.top) + 'px';
